            ...
            <ul class="font-medium flex flex-col md:items-center p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-800 dark:border-gray-700">
                ...
                @else
                    ...
                    @if (Auth::user()->status == "active")
                        ...
                        <!-- Dropdown menu -->
                        <div id="dropdownOrders" class="z-10 hidden font-normal bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 dark:divide-gray-600">
                            <ul class="px-4 py-2 text-sm text-gray-700 dark:text-gray-400 space-y-2" aria-labelledby="dropdownLargeButton">
                                <li>
                                    <x-nav :href="route('dashboard.place_orders')" :active="request()->routeIs('dashboard.place_orders')">
                                        {{ __('Place Orders') }}
                                    </x-nav>
                                </li>
                                @if (Auth::user()->role_id == 1 || Auth::user()->role_id == 4)
                                    <li>
                                        <x-nav :href="route('dashboard.manage_orders')" :active="request()->routeIs('dashboard.manage_orders')">
                                            {{ __('Manage Orders') }}
                                        </x-nav>
                                    </li>
                                @endif
                        ...
                        @if ($ifHasManage)
                            <button id="dropdownNavbarLink" data-dropdown-toggle="dropdownManagers" class="flex items-center justify-between w-full py-2 px-3 text-gray-900 font-semibold rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-gloflow-purple-500 md:p-0 md:w-auto dark:text-white md:dark:hover:text-blue-500 dark:focus:text-white dark:border-gray-700 dark:hover:bg-gray-700 md:dark:hover:bg-transparent">
                                Managers
                                <svg class="w-2.5 h-2.5 ms-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 4 4 4-4"/>
                                </svg>
                            ...
                            <!-- Dropdown menu -->
                            <div id="dropdownManagers" class="z-10 hidden font-normal bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 dark:divide-gray-600">
                                <ul class="px-4 py-2 text-sm text-gray-700 dark:text-gray-400 space-y-2" aria-labelledby="dropdownLargeButton">
                                    ...
                                    @if ($perms->manage_users == 1)
                                        ...
                                        <li>
                                            <x-nav :href="route('dashboard.clients_manager')" :active="request()->routeIs('dashboard.clients_manager')">
                                                {{ __('Clients Manager') }}
                                            </x-nav>
                                        </li>
                                    @endif
